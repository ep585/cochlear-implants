...
class View {
public:
	void Init(const Params &_initialAligment, float (*const &_similarityMetric)(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &)), const char *const &_datasetName);
	~View();
	...
	vec3 Rotation(const int &mouseX, const int &mouseY) const;
	const vec3 &RotationNoInput() const { return params.rotation; }
	vec3 Pan(const int &mouseX, const int &mouseY) const;
	const vec3 &PanNoInput() const { return params.pan; }
	//void RotationMatrix(float out[4][4], const int &mouseX, const int &mouseY) const;
	//void RotationMatrixNoInput(float out[4][4]) const;
	const vec2 &SourceOffset() const { return params.sourceOffset; }
	SDL_Rect ManualCompRect(const int &mouseX, const int &mouseY) const;
	
	void Update();
	
	const float &Centre() const { return centre; }
	const float &Range() const { return range; }
	const int &SamplesN() const { return samplesN; }
	const DrawMode &DrawMode() const { return drawMode; }
	const bool &Plot() const { return plot; }
	const bool &MaskMode() const { return maskMode; }
	const bool &CompRectMode() const { return compRectMode; }
...
private:
	...
	float (*similarityMetric)(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &));
