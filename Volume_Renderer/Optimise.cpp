...
OpVals Scan(float (*const &similarityMetric)(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &)), const char *fileName, const bool &checkFeasible){
	...
	const vec3 &pan0 = ViewManager().PanNoInput();
	const vec3 &rotation0 = ViewManager().RotationNoInput();
	const vec2 &sourceOffset0 = ViewManager().SourceOffset();
	...
	const vec3 panRange = {1.0f, 1.0f, 15.0f};
	//const vec3 rotRange = {0.01f, 0.1f, 0.05f};
	//const vec2 soRange = {5.0f, 5.0f};
	...
	for(int iY=0; iY<n; iY++){
		for(int iX=0; iX<n; iX++){
			...
			if(checkFeasible){
				...
				float rotationMatrix[4][4];
				Render::RotationMatrix(opVals.params.rotation, rotationMatrix);
				Data().Transform3dPoints(opVals.params.pan, rotationMatrix);
	...
	std::cout << "Range covered = " << panRange.xy() << " centered on " << pan0.xy() << "\n";
...
Particle::Particle(float (*const &similarityMetric)(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &)), OpVals &best, const Params &params0, const Params &paramsRange, const bool &checkFeasible){
	Params paramsOff;
	...
	if(checkFeasible){
		...
		do {
			...
			float rotationMatrix[4][4];
			Render::RotationMatrix(current.params.rotation, rotationMatrix);
			Data().Transform3dPoints(current.params.pan, rotationMatrix/*, current.params.sourceOffset*/);
			Data().CalcEnlargedPointPositions();
...
void Particle::Update(float (*const &similarityMetric)(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &)), OpVals &best, const PSParams &psParams, const bool &checkFeasible){
...
void Particle::ReCalc(float (*const &similarityMetric)(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &)), OpVals &best){
	current.f = Render::F(similarityMetric, current.params);
	if(current.f < myBest.f){
		myBest = current;
		if(myBest.f < best.f) best = myBest;
...
OpVals ParticleSwarm(float (*const &similarityMetric)(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &)), const int &iterations, const PSParams &psParams, const Params &params0, const Params &paramsRange, const bool &checkFeasible, Particle *particles/*, const Print &print, int *const &counts, float *const &bests*/){
	...
	if(checkFeasible){
		...
		float rotationMatrix[4][4];
		Render::RotationMatrix(params0.rotation, rotationMatrix);
		Data().Transform3dPoints(params0.pan, rotationMatrix/*, params0.sourceOffset*/);
		Data().CalcEnlargedPointPositions();
		if(Data().OutsideEnlarged()){
...
double TimeDRR(){
	...
	for(int i=0; i<1000; i++){
		...
		float rotationMatrix[4][4];
		Render::RotationMatrix(params.rotation, rotationMatrix);
		
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		
		Render::DrawVolumeRender(params.pan, rotationMatrix, ViewManager().SamplesN(), ViewManager().Range(), ViewManager().Centre(), DrawMode::deep_drr_only/*, params.sourceOffset*/);
		Render::TakeDRR();
	}
...
OpVals LocalSearch(float (*const &similarityMetric)(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &)), const Params &params0, const Params &stepSize0, const char *fileName){
