...
void main(){
	...
	vec4 sum = vec4(0.0);
	//for(int i=-u_samplesN/2; i<u_samplesN/2; i++) sum += vec4(texture3D(u_data, v_start_ctTextureSpace + float(i)*rayJump).r);
	...
	//gl_FragColor = vec4(texture3D(u_data, vec3(v_texCoord, 0.0)).r);
}
