...
void Mask::Init(const int &_width, const int &_height, const int &_maxShapeRadius){
	width = _width;
	height = _height;
...
bool Cell::PosMasked(const int &x, const int &y) const {
	for(int i=0; i<N; i++){
...
bool Mask::PosMasked(const int &x, const int &y) const {
...
void Mask::Draw() const {
	...
	for(int i=0; i<rows*columns; i++) for(int s=0; s<cells[i].N; s++){
		...
		for(int v=0; v<n; v++){
			...
			vertexBuffer[verticesN++] = XRtoScreen(centre + r*(vec2){cosf(angle), sinf(angle)});
			angle -= delta;
			vertexBuffer[verticesN++] = XRtoScreen(centre + r*(vec2){cosf(angle), sinf(angle)});
		}
	}
