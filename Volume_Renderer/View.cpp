...
void View::Init(const Params &_initialAligment,  float (*const &_similarityMetric)(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &)), const char *const &_datasetName){
...
//#ifndef SKIP_OPTIMISE
	...
	*/
	...
	Data().WriteElectrodeRays(params.pan, rotationMatrix, "/Users/eprager/Library/CloudStorage/OneDrive-UniversityofCambridge/CUED/4th Year Project/Output_files/electrodeRays.txt");
...
void View::KeyLineScans(SDL_Event event){
	...
	static float (*const simMetrics[simMetricsN])(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &)) = {
...
// Parameter getters
// ----------
vec3 View::Rotation(const int &mouseX, const int &mouseY) const {
	...
	//vec3 ret = rotation + rotSensitivity*(vec3){0.0f, c*y - s*x, - c*x - s*y};
	vec3 ret = params.rotation + rotSensitivity*(vec3){0.0f, -y, x};
	if(ret.y > M_PI) ret.y = M_PI;
	if(ret.y < -M_PI) ret.y = -M_PI;
	return ret;
}
vec3 View::Pan(const int &mouseX, const int &mouseY) const {
	if(ESDL::GetKeyDown(KEY_MOVE_COMP_RECT) || !leftDown) return params.pan;
	const vec3 ret = params.pan + panSensitivity*(vec3){(float)(mouseX - leftStartX), -(float)(mouseY - leftStartY), 0.0f};
	return ret;
}
SDL_Rect View::ManualCompRect(const int &mouseX, const int &mouseY) const {
