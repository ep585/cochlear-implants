...
// getters
const int &PortWidth(){ return portWidth; }
const int &PortHeight(){ return portHeight; }
const float &PortWidthInverse(){ return portWidthInverse; }
const float &PortHeightInverse(){ return portHeightInverse; }
...
const bool &GetMoveBoundaries(){ return moveBoundaries; }
void SetMoveBoundaries(const bool &value){ moveBoundaries = value; }

static int boundaryOffset = 0;
const int &GetBoundaryOffset(){ return boundaryOffset; }
void SetBoundaryOffset(const int &value){ boundaryOffset = value; }
...
void DrawAxes(const vec3 &origin, const float transformation[4][4], const float &size=1.0f){
	...
	glBufferData(GL_ARRAY_BUFFER, 16*sizeof(float), &fourPositions[0][0], GL_STATIC_DRAW);
...
void DrawVolumeRender(const vec3 &pan, const float rotationMatrix[4][4], const int &samplesN, const float &range, const float &centre, const DrawMode &drawMode/*, const vec2 &sourceOffset*/){
	...
	// uniforms
	glUniformMatrix4fv(UNIFORM(SP::main, U_main::B1), 1, GL_FALSE, &B1[0][0]);
	glUniform1f(UNIFORM(SP::main, U_main::jumpSize), CT().DirVecLength()*nInv);
	glUniformMatrix3fv(UNIFORM(SP::main, U_main::B1_nt), 1, GL_FALSE, &B1_noTranslation[0][0]);
...
//void Render(float (XRData::*const &similarityMetric)(const uint16_t [], const SDL_Rect &) const){
void Render(float (*const &similarityMetric)(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &))){
	...
	const vec3 pan = ViewManager().Pan(mouseX, mouseY);
	float rotationMatrix[4][4];
	RotationMatrix(ViewManager().Rotation(mouseX, mouseY), rotationMatrix);
	//const vec2 sourceOffset = ViewManager().SourceOffset();
	...
	if(saveRects){
		...
		SimMetric::SaveRects(drrName, Array2D<uint16_t>(volumeRenderBuffer, compRect.w, compRect.h),
							 ...
							 [](const int &j, const int &i) -> bool { return XRayMask().PosMasked(compRect.x + i, XRay().Height() - compRect.y - j); });
		saveRects = false;
	}
	...
	if(ViewManager().Plot()){
		const float f = (ViewManager().DrawMode() == DrawMode::static_drr_and_shallow_drr ?
							...
							similarityMetric(Array2D<uint16_t>(volumeRenderBuffer, compRect.w, compRect.h), XRay().ToArrayWithRect(compRect), [](const int &j, const int &i) -> bool { return XRayMask().PosMasked(compRect.x + i, XRay().Height() - compRect.y - j); })
						);
	...
	/*
	{
		...
		const vec3 dims = (vec3){(float)CT().Width(), (float)CT().Height(), (float)CT().Depth()}*CT().Spacing();
	...
	*/
	...
	vec4 xs = {(float)compRect.x, (float)compRect.x, (float)(compRect.x + compRect.w), (float)(compRect.x + compRect.w)};
	...
	vec4 ys = {(float)compRect.y, (float)(compRect.y + compRect.h), (float)compRect.y, (float)(compRect.y + compRect.h)};
...
void CalcValidBoundaries(const vec3 &pan, const float rotationMatrix[4][4]){
	...
	vec3 lll = *(vec3 *)(&bottom[0][0])/bottom[0][3];
	vec3 lhl = *(vec3 *)(&bottom[1][0])/bottom[1][3];
	vec3 llh = *(vec3 *)(&bottom[2][0])/bottom[2][3];
	vec3 lhh = *(vec3 *)(&bottom[3][0])/bottom[3][3];
	vec3 hll = *(vec3 *)(&top[0][0])/top[0][3];
	vec3 hhl = *(vec3 *)(&top[1][0])/top[1][3];
	vec3 hlh = *(vec3 *)(&top[2][0])/top[2][3];
	vec3 hhh = *(vec3 *)(&top[3][0])/top[3][3];
...
//float F(float (XRData::*const &similarityMetric)(const uint16_t [], const SDL_Rect &) const, const Params &params){
...
float F(float (*const &similarityMetric)(const Array2D<uint16_t> &, const Array2D<uint16_t> &, bool (*const &)(const int &, const int &)), const Params &params){
	...
	float rotationMatrix[4][4];
	RotationMatrix(params.rotation, rotationMatrix);
	...
	return similarityMetric(Array2D<uint16_t>(volumeRenderBuffer, compRect.w, compRect.h), XRay().ToArrayWithRect(compRect), [](const int &j, const int &i) -> bool { return XRayMask().PosMasked(compRect.x + i, XRay().Height() - compRect.y - j); });
}

void SaveScreenshot(const char *const &fileName, const SDL_Rect &rect){
